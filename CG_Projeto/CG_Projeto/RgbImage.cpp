...
bool RgbImage::LoadBmpFile( const char* filename )
{
    ...
    if ( bChar=='B' && mChar=='M' ) {            // If starts with "BM" for "BitMap"
        ...
        long offset = readLong(infile);            // Offset to the bitmap table
        long headerSize = readLong(infile);        // Size of the Bitmap header
        NumCols = readLong( infile );
        NumRows = readLong( infile );
        skipChars( infile, 2 );                    // Skip one field (number of color planes)
        int bitsPerPixel = readShort( infile );
        long bytesRead = 30;                        // 2 + 4 + 2 + 2 + 4 + 4 + 4 + 4 + 2 + 2
        long compressionMethod = BI_RGB;
        if (headerSize >= 40) {
    ...
    for ( int i=0; i<NumRows; i++ ) {
        ...
        long k=3*NumCols;                    // Num bytes already read
